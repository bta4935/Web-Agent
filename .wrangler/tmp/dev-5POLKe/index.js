...
// .wrangler/tmp/bundle-g7sXvc/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-g7sXvc/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var Crawler = class {
  ...
  browserEnvBinding;
  options;
  constructor(browserEnvBinding, options = {}) {
  ...
  async _getBrowserPage() {
    const browser = await puppeteer_core_default.launch(this.browserEnvBinding);
    const page = await browser.newPage();
  ...
  async extractHtml(url, extractionOptions) {
    let browser = null;
    try {
      const { browser: launchedBrowser, page } = await this._getBrowserPage();
      browser = launchedBrowser;
      const waitUntil = extractionOptions?.waitUntil || this.options.waitUntil || "networkidle0";
      const timeout = extractionOptions?.timeout || this.options.timeout || 3e4;
      ...
      return { url, status: response?.status?.() ?? 200, html, timestamp: Date.now() };
    } catch (error3) {
      console.error(`Error extracting HTML from ${url}:`, error3);
      return { url, status: 500, error: String(error3?.message || error3), timestamp: Date.now() };
    } finally {
      if (browser) await browser.close();
    }
  }
  async extractText(url, extractionOptions) {
    let browser = null;
    try {
      const { browser: launchedBrowser, page } = await this._getBrowserPage();
      browser = launchedBrowser;
      ...
      return { url, status: response?.status?.() ?? 200, text, timestamp: Date.now() };
    } catch (error3) {
      console.error(`Error extracting text from ${url}:`, error3);
      return { url, status: 500, error: String(error3?.message || error3), timestamp: Date.now() };
    } finally {
      if (browser) await browser.close();
    }
  ...
  async extractBySelector(url, selectors, options) {
    ...
    try {
      ...
      const timeout = options?.timeout || this.options.timeout || 3e4;
          ...
          return selectors2.map((selector) => {
            ...
            const results = nodeList.map((el) => {
              ...
              if (opts?.includeAttributes && Array.isArray(opts.attributes) && opts.attributes.length > 0) {
                attributes = opts.attributes.map((attr) => ({
                  name: attr,
      ...
      return { elements, url, status: response?.status?.() ?? 200, timestamp: Date.now() };
    } catch (error3) {
      console.error(`Error extracting by selector from ${url}:`, error3);
      return { url, status: 500, error: String(error3?.message || error3), timestamp: Date.now() };
    } finally {
      if (browser) await browser.close();
    }
  }
  async extractAfterJsExecution(url, extractionOptions) {
    let browser = null;
    try {
      const { browser: launchedBrowser, page } = await this._getBrowserPage();
      browser = launchedBrowser;
      ...
      const html = await page.content();
      const text = String(await page.evaluate(() => document.body ? document.body.innerText : ""));
      return { url, status: response?.status?.() ?? 200, html, text, timestamp: Date.now() };
    } catch (error3) {
      console.error(`Error extracting after JS execution from ${url}:`, error3);
      return { url, status: 500, error: String(error3?.message || error3), timestamp: Date.now() };
    } finally {
      if (browser) await browser.close();
    }
  ...
  async executeCustomFunction(url, scriptFn, ...args) {
    return { result: null, url, status: 200, timestamp: Date.now() };
  }
};
  ...
  try {
    ...
    if (selectors) {
      ...
      if (outputType === "text" && Array.isArray(extractionResult)) {
        const rawTextForMarkdown = extractionResult.map(
          (el) => Array.isArray(el.results) ? el.results.map((r) => r.text).join(" ") : ""
        ).join("\n\n");
