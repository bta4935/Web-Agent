...
// .wrangler/tmp/bundle-bJ1d7f/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-bJ1d7f/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
__name(extractMetaTags, "extractMetaTags");
async function extractBaseUrl(page) {
  return await page.evaluate(() => {
    const baseElement = document.querySelector("base");
    return baseElement ? baseElement.getAttribute("href") : null;
  });
}
__name(extractBaseUrl, "extractBaseUrl");
async function extractCanonicalUrl(page) {
  return await page.evaluate(() => {
    const canonicalElement = document.querySelector('link[rel="canonical"]');
    return canonicalElement ? canonicalElement.getAttribute("href") : null;
  });
}
__name(extractCanonicalUrl, "extractCanonicalUrl");
...
async function extractTextFromElements(page, selector) {
  return await page.evaluate((selector2) => {
...
__name(extractTextFromElements, "extractTextFromElements");
...
__name(extractTextBySelector, "extractTextBySelector");
async function extractAttributeBySelector(page, selector, attribute) {
  return await page.evaluate((selector2, attribute2) => {
    const elements = Array.from(document.querySelectorAll(selector2));
    return elements.map((el) => el.getAttribute(attribute2)).filter((value) => value !== null);
  }, selector, attribute);
}
__name(extractAttributeBySelector, "extractAttributeBySelector");
...
async function executeCustomJs(page, scriptFn, ...args) {
  const scriptStr = typeof scriptFn === "function" ? scriptFn.toString() : scriptFn;
  return await page.evaluate(scriptStr, ...args);
}
__name(executeCustomJs, "executeCustomJs");
...
function parseCrawlerOptions(request) {
  try {
...
function parseExtractionOptions(request) {
  try {
    const url = new URL(request.url);
...
function parseTextExtractionOptions(request) {
  try {
    const url = new URL(request.url);
    const options = parseExtractionOptions(request);
...
function parseSelectorExtractionOptions(request) {
  try {
    const url = new URL(request.url);
    const options = parseExtractionOptions(request);
...
function parseJsExtractionOptions(request) {
  try {
    const url = new URL(request.url);
    const options = parseExtractionOptions(request);
...
async function handleJsExtraction(request, env2) {
  try {
    ...
    const crawler = new Crawler(env2.BROWSER, crawlerOptions);
    const result = selectors ? await crawler.extractElementsAfterJsExecution(targetUrl, selectors, extractionOptions) : await crawler.extractAfterJsExecution(targetUrl, extractionOptions);
    return new Response(
      JSON.stringify(result),
      { headers: { "Content-Type": "application/json" } }
    );
...
async function handleRequest(request, env2) {
  ...
  try {
    ...
    console.error("Error handling crawler request:", error3);
    return new Response(JSON.stringify({
      error: error3 instanceof Error ? error3.message : "Unknown error"
    }), {
      status: 500,
...
var crawler_default = {
  async fetch(request, env2, ctx) {
    ...
    if (url.pathname === "/" && url.searchParams.has("url") && url.searchParams.has("type")) {
      ...
      return handleRequest(newRequest, env2);
    }
    return new Response("Not Found", { status: 404 });
  }
};
